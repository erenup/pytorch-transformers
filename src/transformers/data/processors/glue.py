...
def glue_convert_example_to_features(example,
    tokenizer,
    max_length=512,
    task=None,
    label_list=None,
...
def glue_convert_examples_to_features(
    ...
    if is_tf_dataset:
        example = processor.get_example_from_tensor_dict(examples)
    ...
    if multi:
        with Pool(cpu_count()) as p:
            annotate_ = partial(glue_convert_example_to_features,
                                tokenizer=tokenizer,
            ...
            features = list(tqdm(p.imap(annotate_, examples), total=len(examples), desc='convert examples to features multipraocessing:{}'.format(cpu_count())))
    else:
        for (ex_index, example) in tqdm(enumerate(examples), total=len(examples), desc='convert examples to features single process'):
            if ex_index % 10000 == 0:
                logger.info("Writing example %d/%d" % (ex_index, len(examples)))
