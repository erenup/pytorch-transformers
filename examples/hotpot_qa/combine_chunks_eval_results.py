    ...
    for _id, example in tqdm(scored_retrieval.items(), total=len(scored_retrieval), desc='convert retrieved hotpot qa to squad format'):
        ...
        if example.get('context', None):
            ...
            if answer in "yes no":
                ...
                answer_start = -1
                is_impossible = False
        if not example.get("scored_retrieved", None):
            print('id: {} miss'.format(_id))
            continue
...
def combine_examples_with_scores(hotpot_examples, examples_dict):
    ...
    for example in hotpot_examples:
        ...
        example[module] = []
        if not paras_ranked:
            print('id {} misses paras!'.format(_id))
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_dir', default='', help='evaluated file')
    ...
    chunks = glob.glob('{}/chunk*'.format(args.eval_dir))
    print(chunks)
    all_examples = []
    for chunk in chunks:

        eval_file = os.path.join(chunk, 'dev.json.para')
        result_file = os.path.join(chunk, 'eval_results.json')
        print('processing {}, {}'.format(eval_file, result_file))
        chunk_examples = read_line_json(eval_file)
        chunk_results = read_json(result_file)
        chunk_examples = combine_examples_results(chunk_examples, chunk_results)
        all_examples.extend(chunk_examples)

    hotpot_examples = read_json(args.hotpot_file)
    examples_dict = examples2dict(all_examples)
    ...
    for _id, example in hotpot_dict.items():
        ...
        if retrieved is not None:
            ...
            print('id: {} misss retrieved scored data!'.format(_id))

    squad_data = convert_topn2squad(hotpot_dict, top_n=args.top_n)

    output_file = os.path.join(args.eval_dir, "combined.ranked")
